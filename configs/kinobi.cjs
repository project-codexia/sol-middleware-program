...
// Instantiate Kinobi.
const kinobi = k.createFromIdls([path.join(idlDir, "middleware_program.json")]);
...
kinobi.update(
  new k.bottomUpTransformerVisitor([{
    select: ['[programNode]', node => 'name' in node && node.name === "middlewareProgram"],
    transform: (node) => {
...
kinobi.update(
  new k.updateAccountsVisitor({
    ...
    assetSigner: {
      size: 0,
      seeds: [
        k.constantPdaSeedNodeFromString("middleware-program-execute"),
